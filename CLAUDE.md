...
- ✓ **Engine sanity v2** (2026-04-20) : 23 tests — exécution (9) + remaining detectors (7) + risk/TF/integrity (7). **23/23 PASS**. Bloc A exécution : SL/TP fills ✓, intrabar SL-before-TP priority ✓, trailing ratchet ✓, breakeven ✓, time-stop SCALP ✓, SHORT mirror ✓, costs wire ✓. Bloc B detectors : IFVG ✓, BOS ✓, EMA cross ✓, VWAP bounce ✓, RSI extreme ✓, ORB ✓. Bloc C risk : cooldown 5min ✓, session cap 10 ✓, daily kill-switch -4R ✓, denylist ✓, position size cap (AGGRESSIVE×A=1.5 → 333sh) ✓, TFA 15m gap-merge ✓, TZ coherence parquet UTC vs 9:30 ET ✓. **Finding majeur : `detect_order_blocks` structurellement cassé** ([order_block.py:40-45](backend/engines/patterns/order_block.py#L40-L45)) — `window = candles[-lb:]` inclut la last candle, donc `close > swing_high` mathématiquement impossible. Empirique oct_w2 SPY 1m : **0 signals sur 1999 bars**. Fix 1 ligne connu (`candles[-(lb+1):-1]`) mais non appliqué (hors scope sanity, changerait le corpus). Test B2 asserte le comportement cassé comme regression marker. **v1 + v2 = 29/29 logic-correct. Moteur entièrement auditable.** Détails [engine_sanity_v2_verdict.md](backend/data/backtest_results/engine_sanity_v2_verdict.md).
