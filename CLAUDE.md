...
- ⏸ **Phase B/C/Survivor STOP — pivot décision** : aucun signal ni combinaison n'a d'edge sur 4 semaines sous caps prod. Recommandation : **(B) signal redesign / détecteur audit** (MASTER alignment — D/4H bias gate audit, TF mismatch audit, détecteurs compression 5m→1m). Alternatives : (A) paper-trade le best 3-pack comme baseline instrumenté (pas pour P&L), ou (C) Polygon 18 mois pour vérifier si 4 semaines insuffit à détecter un edge fin (priors suggèrent non — null monotone sur 3 échelles : fair audit → calibration → portfolio).
