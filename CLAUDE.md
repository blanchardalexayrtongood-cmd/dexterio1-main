...
- ✓ **Engine sanity v1** (2026-04-20) : 6 tests algorithmiques read-only sur oct_w2 (1999 1m SPY bars). **5/6 PASS** : OHLC identity ✓, FVG definition ✓ (0 false positives), engulfing 1:1 ✓, sweep zero false positives ✓, required_signals gate ✓. **Test 5 FAIL** : TimeframeAggregator silently merges ~5.7% of 5m bars (24/420 on the week) when the `minute%5==4` 1m bar is missing from feed — `_build_multi_timeframe_candles` pandas path is dead code (pandas 3.x rejects `5T`). Impact modéré sur fidélité HTF, pas root cause du 27-playbook null. Détails [engine_sanity_v1_verdict.md](backend/data/backtest_results/engine_sanity_v1_verdict.md). **Moteur honnête** — le bottleneck est signal/TP, pas parsing.
