...
- ⏸ **5 phases monotonic null — pivot décision** : fair audit → 3 single-signal calibrations → portfolio → bias gate. Aucune n'a produit E[R]>0. Recommandation : **(A) TF compression / détecteur code review** (lire les 7 MASTER faithful detectors vs MASTER specs, vérifier que les implémentations correspondent au framework D/4H bias → 15m/5m setup → 1m execution). Alternatives : (B) re-audit bias avec market_state.py `detect_structure` au lieu de SMA_5 proxy, ou (C) accepter détecteurs sans edge → paper baseline OU Polygon 18m.
