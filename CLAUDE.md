...
- ⏸ **6 phases — pivot décision (revised)** : 5 phases ont produit null monotone sur l'univers testé (Families C/E + 7 vocab-borrowing). D.2 révèle que 3/6 MASTER families sont untested. Recommandation : **(A) Instancier Aplus_03 (IFVG Flip)** — untested MASTER family, code primitives existent (ifvg.py), est. 2-4h, peut réutiliser calib_corpus_v1. Génère un signal propre : si négatif → bear case renforcé pour MASTER intraday SPY/QQQ ; si positif → path ignorée trouvée. Alternatives : (B) restructure 4 gate-OFF YAMLs pour activer HTF bias gate puis re-test (cheap mais incremental, asymptote attendu) ; (C) accept détecteurs sans edge → paper baseline OU Polygon 18m.
